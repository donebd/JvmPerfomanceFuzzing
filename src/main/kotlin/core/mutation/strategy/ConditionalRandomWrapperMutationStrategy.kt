...
/**
 * Стратегия мутации, которая оборачивает отдельные операторы или блоки кода в условные
 * конструкции. Создает эквивалентные по функциональности варианты кода с добавлением
 * условных ветвлений, что позволяет тестировать эффективность предсказания ветвлений
 * и оптимизации условных конструкций в различных JVM.
 *
 * Стратегия поддерживает два режима мутации:
 * 1. Оборачивание отдельного оператора в условие с дублированием оператора в обеих ветвях
 * 2. Оборачивание блока кода в условную конструкцию с предсказуемым условием
 *
 * Генерируемые условные конструкции сохраняют исходную функциональность кода, поскольку
 * либо выполняется оригинальный код, либо его идентичная копия.
 */
